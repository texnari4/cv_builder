...
Чтобы включить/отключить редактирование, установите в Script Properties DISABLE_EDITING = 1 (включено) или 0 (выключено), либо вызовите setDisableEditing(true|false) из редактора скрипта (только владелец может менять).
При включённом флаге редактирование блокируется для всех, после изменения флага требуется передеплоить проект. 
В таком варианте использования, вы сможете делиться своим резюме по ссылке
